...
You're given 4 business questions. Write the SQL to answer each one using the normalized schema above.

Run the queries, measure performance, and identify bottlenecks.
...
Document: How many tables do you join?

Measure: What's the query execution time?

Analyze: Why is this slow? (Hint: JOIN chain + GROUP BY)
...
Document: How do you join diagnosis and procedure data across separate junction tables?

Measure: What's the query execution time?

Analyze: Why is this slow? (Hint: Two junction tables → row explosion)
...
Write the SQL query (you'll need a self-join on encounters)

Document: How do you detect a readmission using discharge_date and subsequent encounters?

Measure: What's the query execution time?

Analyze: Why is this slow? (Hint: Self-joins on large tables)
...
Document: What's the JOIN chain? (billing → encounters → providers → specialties)

Measure: What's the query execution time?

Analyze: Why is this slow? (Hint: Multiple JOINs + aggregation)
...
The SQL (optimized for star schema)

Execution time estimate (e.g., "-150ms vs. 1.8s original")

Improvement factor (e.g., "12x faster")

Explanation: Why is it faster? (fewer joins? pre-aggregated? denormalization? better indexes?)
